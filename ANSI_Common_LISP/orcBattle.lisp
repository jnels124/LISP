;; orc battle Land of LISP chapter 9 
...
;; Initialize monsters, start game, determine victor at end
(defun orc-battle ()
  (init-monsters)
  (init-player)
  (game-loop)
...
;;game loop. Handles a round of battle
(defun game-loop ()
...
(defun init-player ()
  ...
  (setf *player-agility* 100)
  (setf *player-strength* 100))
...
;; Heler function to generate random value
(defun randval (n) 
  (1+ (random (max 1 n))))
...
;; Allows player to target a specific monster 
(defun pick-monster () 
  (fresh-line)
  (princ "Monster #:")
  (let ((x (read)))
...
;; Create and display list of monsters
(defun show-monsters ()
  (fresh-line)
  (princ "Your foes:")
  (let ((x 0))
...
;; define generic functions to display monster info 
